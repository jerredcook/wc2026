...
- **Add to calendar** — export the whole tournament (📅 Calendar) or a single nation's matches (from its team card) as a standard `.ics` file with correct UTC kickoff instants, so the games land at the right local time in Apple / Google / Outlook calendars. Played matches carry the result in the event notes.
...
- **Pick'em** — predict every match (Home / Draw / Away, or the winner in knockouts), optionally call the **exact score**, set a **confidence (×1–3)** on each pick, and name your champion. Calls score themselves as results land on a points system — **+1** correct result, **+2** exact score, **×** your confidence, and a small cost for confident misses (`−(confidence−1)`) — so bold calls cut both ways. Correct picks turn green, wrong ones strike through red with the actual outcome dash-outlined, above your points total, accuracy %, exact count, and a **per-round breakdown** (Group / R32 / … / Final). One-tap shareable summary and a reset; it's all saved on your device (in your saved preferences) — no account, nothing sent anywhere.
