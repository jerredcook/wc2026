...
- **Card tracker** — every booking with **who got it, the minute, and the game**, in five views: **by team** (a discipline leaderboard — tap a row to expand the team's full card list, each card tappable into its match), **by player** (the most-carded players with their complete card history, plus a **⚠ "on a booking" watch** for players one yellow from a suspension whose team is still alive — two yellows in separate games before the post-quarterfinal wipe means a one-match ban), **fair play** (FIFA fair-play points — yellow −1, second-yellow −3, direct red −4 — the actual group tiebreaker, most-penalised first), **by round**, and **by match** (most-booked games with each card's minute and player inline, reds weighted, tappable into the timeline). Built from the same goals/cards timeline already on each match — no extra requests — and knockout cards are counted exactly once (a long-standing double-count via the bracket/fixture overlap is fixed).
...
- **Team rankings — kickoff vs. now** — all 48 qualified nations listed by FIFA World Ranking, filterable by confederation. Each row shows the ranking **at the World Cup's start** (June 11 — the official pre-tournament release, used for seeding) *and* **today's** ranking, with the movement between them (▲ up / ▼ down). FIFA only *publishes* on fixed dates (nothing mid-tournament; the next release lands the day after the Final), so "now" is computed the way FIFA will: the official June-11 **points** for all 211 nations (pulled from FIFA's own feed by the daily Action into [`data/rankings.json`](data/)) plus **FIFA's SUM rating formula applied to every completed match** — win/draw/loss and shootout weightings, match-importance coefficients (50 through the R16, 60 from the QF), the no-points-lost-in-knockouts rule — re-ranked across all 211 nations, updating the moment a match ends. The tab is ordered by today's ranking and shows each team's **points** (with the change since June 11), so you can see how close the top really is — and **every column header sorts the table** (tap again to flip direction). The daily fetch **probes FIFA's release id-space** rather than trusting the ranking page (whose embedded release list has been observed ~10 months stale), so the post-Final official release is picked up automatically and confirms the computed table. And the **rank chip beside every team name** — in the schedule, the Today & Live bar, the standings and the stadium grid — shows the **current** rank with the **kickoff** rank in parentheses whenever it has moved: *Spain #1(2)* (now 1st, seeded 2nd), *Brazil #6(5)* (slipped one). The current number is green when the team has climbed and red when it has dropped; teams that haven't moved just show a plain rank. So the current ranking travels with the team everywhere, not just in the rankings table. Team profile cards show both ranks too. (The fetch script takes `--women`, ready for the Women's World Cup edition.)
