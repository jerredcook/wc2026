...
- **Card tracker** — yellow & red cards aggregated **by team** (a discipline leaderboard, sorted by reds then yellows, each team tappable for its profile) or **by round**, with running totals — built from the same goals/cards timeline already on each match, so no extra requests.
